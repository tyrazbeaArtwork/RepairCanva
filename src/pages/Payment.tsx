...
import { Copy, Lock, ArrowLeft, Check, MessageCircle, CreditCard, QrCode, Sparkles } from "lucide-react";
...
// ============================================
const SELLER_CONFIG = {
  // Payment amounts by package code (use in URL: /payment?package=basic)
  packages: {
    basic: { name: "Basic Package", amount: 50 },
    standard: { name: "Standard Package", amount: 150 },
...
type PaymentMode = "priced" | "flexible";
...
const Payment = () => {
  ...
  // Determine payment mode and amount from URL
    ...
    <div className="min-h-screen bg-background relative">
      ...
      <div className="flex flex-col min-h-screen max-w-[480px] mx-auto bg-background relative z-10 sm:my-8 sm:rounded-3xl sm:border sm:border-border/50 sm:shadow-2xl sm:overflow-hidden">
        ...
        <main className="flex-grow p-6 pb-32">
          {/* Amount Display - Only show for priced mode */}
          {mode === "priced" && (
            <motion.div
            ...
            >
              ...
              <div className="flex items-baseline justify-center gap-1">
                <span className="text-2xl font-medium text-muted-foreground">{SELLER_CONFIG.currency}</span>
                <span className="text-5xl font-extrabold text-foreground tracking-tight">{amount.toFixed(2)}</span>
              </div>
            </motion.div>
          )}

          {/* Flexible Mode Header */}
          {mode === "flexible" && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
